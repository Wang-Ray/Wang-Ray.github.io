...
```java
`@EnableApolloConfig({"application","3721zh.parent"})`
```
...
```yaml
`app:   id: smart-order-service apollo:   meta: http://192.168.1.238:8080   cacheDir: ../data/apolloCache   bootstrap:     enabled: true`
```

或者application.properties

```properties
`app.id=smart-order-service # set apollo meta server address, adjust to actual address if necessary apollo.meta=http://192.168.1.238:8080 apollo.cacheDir=../data/apolloCache apollo.bootstrap.enabled=true`
```
...
export ENV=dev
export APOLLO_META=http://192.168.1.194:8080
```
...
export ENV=fat
export APOLLO_META=http://192.168.1.236:8080
```
