...
categories: 综合
tags: charset encoding comprehensive percent-encoding "url encoding"
---
...
### URL 编码（百分号编码 Percent-Encoding）

✅ 是什么

URL 里**不能直接放中文、空格、特殊符号**，要转换成 `%两位十六进制` 的格式，就是 URL 编码。 标准：**RFC 3986**

✅ 核心规则

1. 英文字母 `A-Z a-z`、数字 `0-9`、`- _ . ~` → **不用编码**
2. 其余字符：先转 UTF-8 字节，再每个字节写成 `%XX`
...
原文：`name=张三&age=20` UTF-8 编码后： `name=%E5%BC%A0%E4%B8%89&age=20`

✅ 两种易混淆场景

1. **Query 参数（? 后面）**：`& =` 必须编码，防止截断
2. **路径部分（/ 之间）**：`/ ? #` 必须编码

> ⚠️ 不要手动拼接字符串！各语言都有内置函数自动编码：
>
> - JS：`encodeURIComponent()`（参数） / `encodeURI()`（完整 url）
> - Python：`urllib.parse.quote`
> - Java：`URLEncoder.encode(str, "UTF-8")`

✅ 补充误区

`encodeURI` 和 `encodeURIComponent` 不一样：

- `encodeURI`：保留 `:/?#&=`，适合**完整 URL**
- `encodeURIComponent`：全部特殊符号编码，适合**单个参数值**（推荐传参使用）
