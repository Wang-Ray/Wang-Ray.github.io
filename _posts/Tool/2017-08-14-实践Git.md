...
**注意：**避免Windows和Unix换行符不同而引起比对差异问题（详见[GitHub 第一坑：换行符自动转换](http://blog.jobbole.com/46200/)），建议安装Git for Windows过程中，windows选择Checkout Windows-style line endings, commit Unix-style line endings，unix选择Checkout-as-is, commit Unix-style line endings，参见下图：
...
| 值      | 行为说明                                                     |
| ------- | ------------------------------------------------------------ |
| `true`  | 检测到**不可逆转换 / 混合换行符** → **直接拒绝提交**，抛出 fatal 错误 |
| `warn`  | 检测到问题 → **输出警告，但允许提交**（推荐团队初期使用）    |
| `false` | 关闭校验，不管混合换行符，直接提交（风险高）                 |
