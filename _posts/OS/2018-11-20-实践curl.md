...
$ curl -X POST --header 'Content-Type: application/json' --header 'Accept: application/json' 'http://192.168.1.192:8080/test/test?param=%E6%B5%8B%E8%AF%95'
```
...
```shell
$ curl -H 'Content-Type:application/xml' -d 'reqXML=abc' http://test.com/
```



```shell
$ curl -X POST -F "bigfile=@src/main/resources/bigfile" --limit-rate 5k http://localhost:8080/async-read
```

### redirect

```shell
# 302重定向到https://www.baidu.com
